...
const Header = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'glass py-[15px]' : 'bg-transparent py-[15px]'}`}>
          ...
          >
            <div className="flex items-center justify-center transition-all duration-300 group-hover:scale-105">
              <img
                ...
                className="h-[45px] sm:h-[48px] lg:h-[50px] w-auto flex-shrink-0 object-contain drop-shadow-sm transition-opacity duration-300 group-hover:opacity-90"
                loading="eager"
                decoding="async"
              />
            </div>
    ...
    <section id="faq" className="py-24 lg:py-32 bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <AnimatedList
          ...
          onItemSelect={(_, index) => toggleFAQ(index)}
          renderItem={(item, index, isSelected) => (
            <div
              className={`border transition-all duration-300 rounded-2xl overflow-hidden bg-white ${isSelected ? 'border-navy-900 shadow-md' : 'border-navy-100'}`}
            >
              <button
              ...
              >
                <span className={`font-display font-semibold transition-colors pr-4 ${openIndex === index ? 'text-gold-500' : 'text-navy-900'}`}>{item.question}</span>
                <motion.svg
                  animate={{ rotate: openIndex === index ? 180 : 0 }}
                  transition={{ duration: 0.2 }}
                  className={`w-5 h-5 flex-shrink-0 ${openIndex === index ? 'text-gold-500' : 'text-navy-500'}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <AnimatePresence>
                {openIndex === index && (
                  ...
                  >
                    <div className="px-6 pb-5 text-navy-600 leading-relaxed border-t border-navy-100 pt-4 bg-navy-50/30">
                      {item.answer}
                    </div>
                  </motion.div>
                )}
